...
async function setupForExistingUser() {
  ...
  try {
    ...
    const testUser = await usersCollection.findOne({ email: 'billing-test-1771698236603@example.com' });
    ...
    if (existing) {
      console.log('✅ Billing exists, updating...');
    ...
    if (!existingAccount) {
      ...
      const accountDoc = {
        ...
        userId: testUser._id,
        platform: 'twitter',
        accountName: 'LoadTestAccount',
        accountId: 'loadtest123',
    ...
    console.log(`\nℹ️  Use these credentials for load test:`);
