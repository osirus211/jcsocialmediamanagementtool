 ...
 * 
 * Processes jobs from mastodon_publish_queue
 * Publishes posts to Mastodon using MastodonPublisher
 */
...
const QUEUE_NAME = 'mastodon_publish_queue';
const PLATFORM = 'mastodon';

export interface MastodonPublishJobData {
  postId: string;
...
export class MastodonPublisherWorker {
  private worker: Worker<MastodonPublishJobData> | null = null;
  private publisher: MastodonPublisher;
  ...
  start(): void {
    if (this.worker) {
      logger.warn('Mastodon publisher worker already running');
      return;
    }

    const redis = getRedisClient();

    this.worker = new Worker<MastodonPublishJobData>(
      QUEUE_NAME,
      async (job: Job<MastodonPublishJobData>) => {
    ...
    this.worker.on('completed', (job) => {
      logger.debug('Mastodon publish job completed', {
        jobId: job.id,
    ...
    this.worker.on('failed', (job, error) => {
      logger.error('Mastodon publish job failed', {
        jobId: job?.id,
        postId: job?.data.postId,
        error: error.message,
      });
    ...
    logger.info('Mastodon publisher worker started', {
      queue: QUEUE_NAME,
      concurrency: 5,
      rateLimit: '10/sec',
    });
  ...
  async stop(): Promise<void> {
    if (this.worker) {
      ...
      this.worker = null;
      logger.info('Mastodon publisher worker stopped');
    }
  }

  private async process(job: Job<MastodonPublishJobData>): Promise<void> {
    ...
    try {
      ...
      logger.error('Mastodon post publishing failed', {
        postId,
