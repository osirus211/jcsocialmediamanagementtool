 ...
 * 
 * Processes jobs from pinterest_publish_queue
 * Publishes posts to Pinterest using PinterestPublisher
 */
...
const QUEUE_NAME = 'pinterest_publish_queue';
const PLATFORM = 'pinterest';

export interface PinterestPublishJobData {
  postId: string;
...
export class PinterestPublisherWorker {
  private worker: Worker<PinterestPublishJobData> | null = null;
  ...
  start(): void {
    if (this.worker) {
      logger.warn('Pinterest publisher worker already running');
      return;
    }

    const redis = getRedisClient();

    this.worker = new Worker<PinterestPublishJobData>(
      QUEUE_NAME,
      async (job: Job<PinterestPublishJobData>) => {
    ...
    this.worker.on('completed', (job) => {
      logger.debug('Pinterest publish job completed', {
        jobId: job.id,
    ...
    this.worker.on('failed', (job, error) => {
      logger.error('Pinterest publish job failed', {
        jobId: job?.id,
        postId: job?.data.postId,
        error: error.message,
      });
    ...
    logger.info('Pinterest publisher worker started', {
      queue: QUEUE_NAME,
      concurrency: 5,
      rateLimit: '10/sec',
    });
  ...
  async stop(): Promise<void> {
    if (this.worker) {
      ...
      this.worker = null;
      logger.info('Pinterest publisher worker stopped');
    }
  }

  private async process(job: Job<PinterestPublishJobData>): Promise<void> {
    ...
    try {
      ...
      logger.info('Pinterest post published successfully', {
        postId,
      ...
      logger.error('Pinterest post publishing failed', {
        postId,
