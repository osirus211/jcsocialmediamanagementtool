 ...
 * 
 * Processes jobs from bluesky_publish_queue
 * Publishes posts to Bluesky using BlueskyPublisher
 */
...
import { BlueskyPublisher } from '../providers/publishers/BlueskyPublisher';
...
const QUEUE_NAME = 'bluesky_publish_queue';
const PLATFORM = 'bluesky';

export interface BlueskyPublishJobData {
  postId: string;
...
export class BlueskyPublisherWorker {
  private worker: Worker<BlueskyPublishJobData> | null = null;
  private publisher: BlueskyPublisher;
  ...
  start(): void {
    if (this.worker) {
      logger.warn('Bluesky publisher worker already running');
      return;
    }

    const redis = getRedisClient();

    this.worker = new Worker<BlueskyPublishJobData>(
      QUEUE_NAME,
      async (job: Job<BlueskyPublishJobData>) => {
    ...
    this.worker.on('completed', (job) => {
      logger.debug('Bluesky publish job completed', {
        jobId: job.id,
    ...
    this.worker.on('failed', (job, error) => {
      logger.error('Bluesky publish job failed', {
        jobId: job?.id,
        postId: job?.data.postId,
        error: error.message,
      });
    ...
    logger.info('Bluesky publisher worker started', {
      queue: QUEUE_NAME,
      concurrency: 5,
      rateLimit: '10/sec',
    });
  ...
  async stop(): Promise<void> {
    if (this.worker) {
      ...
      this.worker = null;
      logger.info('Bluesky publisher worker stopped');
    }
  }

  private async process(job: Job<BlueskyPublishJobData>): Promise<void> {
    ...
    try {
      ...
      logger.error('Bluesky post publishing failed', {
        postId,
