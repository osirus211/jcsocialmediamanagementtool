 ...
 * 
 * Processes jobs from linkedin_publish_queue
 * Publishes posts to LinkedIn using LinkedInPublisher
 */
...
export class LinkedInPublisherWorker {
  private worker: Worker<LinkedInPublishJobData> | null = null;
  private publisher: LinkedInPublisher;
  ...
  start(): void {
    if (this.worker) {
      logger.warn('LinkedIn publisher worker already running');
      return;
    }

    const redis = getRedisClient();

    this.worker = new Worker<LinkedInPublishJobData>(
      QUEUE_NAME,
      async (job: Job<LinkedInPublishJobData>) => {
    ...
    logger.info('LinkedIn publisher worker started', {
      queue: QUEUE_NAME,
      concurrency: 3,
      rateLimit: '5/sec',
    });
  ...
  async stop(): Promise<void> {
    if (this.worker) {
      ...
      this.worker = null;
      logger.info('LinkedIn publisher worker stopped');
    }
  }

  private async process(job: Job<LinkedInPublishJobData>): Promise<void> {
    ...
    try {
      ...
      logger.error('LinkedIn post publishing failed', {
        postId,
        error: error.message,
